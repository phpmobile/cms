var v={
	group:"1",
	init:function(id,group1){
		this.group=id+"_group";
        $.formValidator.initConfig({
			formID:id,
			//theme:'ArrowSolidBox',
			mode:'AutoTip',
			validatorGroup:this.group,
			//submitOnce:true,
	...
	text:function(id,name){		
		$("#"+id).after("<span id='"+id+"_tip_outer'></span>");
		var arr=[0,100];
		switch(name){
			case  "bt"      :name="标题";            arr=[1,50]; break;		
			case  "jgmc"    :name="机构名称";         arr=[1,50]; break;	
			case  "kcmc"    :name="课程名称";         arr=[1,50]; break;	
			case  "dbyx"    :name="代表院校";         arr=[1,50]; break;	
			case  "jc"      :name="简称";            arr=[1,30]; break;
			case  "lxr"     :name="联系人";           arr=[2,20]; break;
			case  "dz"      :name="地址";            arr=[2,100]; break;
			case  "xm"      :name="姓名";            arr=[2,40]; break;
			case  "nc"      :name="昵称";            arr=[2,40]; break;
			case  "hwbsc"    :name="办事处名称";       arr=[1,50]; break;	
			case  "kksj"    :name="开课时间";         arr=[2,40]; break;
			case  "jdyx"    :name="就读院校";         arr=[2,40]; break;
			case  "jdzy"    :name="就读专业";         arr=[2,40]; break;
			case  "time"    :name="时间";            arr=[2,40]; break;	
			case  "xcmc"    :name="相册名称";         arr=[2,40]; break;	
			case  "xcms"    :name="相册描述";         arr=[0,400]; break;	
			case  "ly"      :name="来源";            arr=[1,50]; break;	
			case  "zw"      :name="正文";            arr=[0,400]; break;	
			case  "szqy"    :name="所在企业";        arr=[2,40]; break;	
			case  "zhw"      :name="职位";           arr=[2,40]; break;
			default:name=name;
		}
		$("#"+id).formValidator({
			validatorGroup:this.group,
			...
			min:arr[0],
			max:arr[1],
			onError:name+"为"+arr[0]+"-"+arr[1]+"个字符"
		});
	},
	...
	num:function(id,name){
		$("#"+id).after("<span id='"+id+"_tip_outer'></span>");
		var arr=[0,100];
		switch(name){
			case  "mz"      :name="数字";            arr=[1,1000000]; break;
			case  "bfb"      :name="数字";            arr=[1,100]; break;
			case  "mzfk"      :name="数字";            arr=[1,1000000]; break;			
			default:name=name;
		}
		$("#"+id).formValidator({
			validatorGroup:this.group,
			...
			max:arr[1],
			type:"number",
			onError:name+"范围为"+arr[0]+"-"+arr[1]
		});
	},
	reg:function(id,name){
		$("#"+id).after("<span id='"+id+"_tip_outer'></span>");
		var reg;
		switch(name){
			case  "qq"            :name="QQ号";      reg="^[1-9]+[0-9]{4,15}$"; break;	
			case  "tel_area"      :name="区号";      reg="^\\d{3,4}$"; break;	
			case  "tel_number"      :name="号码";      reg="\\d{7,8}$"; break;	
			case  "email"        :name="邮箱";      reg="^\\w+((-\\w+)|(\\.\\w+))*\\@[A-Za-z0-9]+((\\.|-)[A-Za-z0-9]+)*\\.[A-Za-z0-9]+$"; break;
			case  "mobile"      :name="移动电话";      reg="^([0-9-]{10,13})$";//"^(13[0-9]{9}|15[012356789][0-9]{8}|18[0256789][0-9]{8}|147[0-9]{8})$"; break;//reg="^1[0-9]{10}$"; break;
			case  "mobile_2"      :name="移动电话";    reg="^([0-9-]{10,13})$"; break;//reg="^[1-9]([0-9-]{10,12})$"; break;
			case  "IDcard"      :name="身份证号";     reg="^(\\d{14}|\\d{17})(\\d|[xX])$"; break;// reg="^[1-9]([0-9]{14}|[0-9]{17})$"; break;
			case  "Skype"      :name="Skype号";      reg="^[A-Za-z]([A-Za-z0-9_]){6,40}$"; break;//reg="^.{1,50}$"; break;
			case  "yyzz"      :name="营业执照编号";      reg="^[A-Za-z0-9]{1,20}$"; break;
            case  "pwd"      :name="密码";      reg="^(\\S){6,20}$"; break;
			default:name=name;
		}	
		$("#"+id).formValidator({
			validatorGroup:this.group,
	...
	pwd1:function(id){
		...
		$("#"+id).formValidator({
			...
			onError:"密码长度不能低于6位字符"
		});
	},
	pwd2:function(id,id1){
		$("#"+id).after("<span id='"+id+"_tip_outer'></span>");
		$("#"+id).formValidator({
            validatorGroup:this.group,
            onShow:false,
            onFocus:"请输入确认密码，长度为6-20个字符",
            onCorrect:"密码一致",
            tipID:id+"_tip_outer"
		}).inputValidator({
			min:6,max:20,
			empty:{
				leftEmpty:false,
				rightEmpty:false,
				emptyError:"确认密码不能包含空格"
			},
			onError:"密码长度不能低于6位字符"
			}).compareValidator({desID:id1,operateor:"=",onError:"输入的密码不一致,请确认"});
	},	
	title:function(id){
		$("#"+id).formValidator({
			validatorGroup:this.group,
    ...
    mobile:function(id,url){
        ...
        $("#"+id).formValidator({
            ...
            success : function(data){
                if( data.indexOf("此手机可以注册!") > 0 ) return true;
                if( data.indexOf("此手机已存在,请填写其它手机!") > 0 ) return false;
                return false;
            },
            buttons: $("#button"),
            error: function(jqXHR, textStatus, errorThrown){alert("服务器没有返回数据，可能服务器忙，请重试"+errorThrown);},
            onError : "该手机已经注册，请使用其它手机号码",
...
$.formValidator = 
{
	...
	initConfig : function(controlOptions)
	{
		...
		//如果填写了表单和按钮，就注册验证事件
		if(settings.formID!=""){
		    if($("#"+settings.formID).has("input[type='submit']").length>0)
			{
			...
			else if($("#"+settings.formID+"_submit").length>0)
			{
				$("#"+settings.formID+"_submit").live("click",function(){
					if(settings.ajaxForm!=null){
						$.formValidator.ajaxForm(settings.validatorGroup,settings.ajaxForm);
						return false;
					}
