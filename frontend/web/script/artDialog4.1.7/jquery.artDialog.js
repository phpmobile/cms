var d={
    ...
    tip:function(a1,a2,a3){
        var content,icon,fun;
        var what=function(val){
            if(val=="suc"||val===true||val==1){
                content="操作成功";
                icon="suc";
            }else if(val=="err"||val===false||val==0){
                content="操作失败";
                icon="err";
            }else if(val=="del"){
                content="删除成功";
	...
	box:function(titleTxt,contentId,okfun,id) {
		okfun?cancel=true:cancel=null;
		okfun=okfun?okfun:false;
		if(!id){
			id='artDialog_open';
		}		
		return art.dialog({
	...
	url:function(title,url,id,wd,ok){
		if(!id){
			id='artDialog_open';
		}
		if(!wd)
	...
	doing:function(content){
		if(!content){
			content="正在处理..";
		}
		return art.dialog({
			content:content+'<div class="aui_loading"><span>loading..</span></div>',
			title:false,
			fixed: true,
			lock: true,
                        cancel:false,
...
function openbox(contentId,titleTxt,okfun) {
    okfun?cancel=true:cancel=null;
    okfun?okfun=eval(okfun):okfun=false;
    return artDialog({
        id: 'artDialog_open',
        title: titleTxt,
        lock:true,
...
function openurl(title,url)
{
    ...
    $.ajax({
        ...
        success   : function (data,textStatus) {
            //$("body").append('<div id="temp_box" style="display:none">'+ data +'</div>');
            //open("temp_box","ajax 请求内容");
            dialog.content(data);
        }
    });
    return dialog;
